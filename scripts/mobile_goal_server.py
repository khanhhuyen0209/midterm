...
from beginner_tutorials.srv import turtleGoal, turtleGoalResponse
...
def odom_callback(msg):
    global current_pose
    current_pose = msg.pose.pose  # Store only the pose
...
def go_to_goal(req):
    ...
    while not rospy.is_shutdown():
        ...
        current_theta = get_yaw_from_quaternion(current_pose.orientation)
        ...
        if distance < 0.1:
            ...
            vel_msg.angular.z = 0
            pub.publish(vel_msg)
            return turtleGoalResponse(success=True)
...
def go_to_goal_server():
    ...
    rospy.Service('go_to_goal', turtleGoal, go_to_goal)
    rospy.spin()
